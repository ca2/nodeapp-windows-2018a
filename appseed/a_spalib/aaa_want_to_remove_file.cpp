...
void file_put_contents_dup(const char * path, const char * contents)
{
...
int file_length_dup(const char * path)
{
   FILE * f = fopen(path, "rb");
   if(!f)
      return -1;
...
string file::title(const char * path)
{
   string str(path);
   size_t iPos;
...
string file::name(const char * path)
{
   string str = title(path);
      ...
      try
      {
         ...
         if(strVersion == "fileset v1")
         {
            while(true)
            {
               ...
               string strPath = dir::path(pszDir, strRelative.c_str());
...
*/
  ...
  /* void file::write_ex1_string(ex1::file * pfile, MD5_CTX * pctx, string & str)
   ...
   void file::read_ex1_string(FILE * pfile, class md5 * pctx, string & str)
   {
      int iLen;
   ...
   bool PrintModules(string & strImage, DWORD processID, const char * pszDll )
{
    HMODULE hMods[1024];
    HANDLE hProcess;
    DWORD cbNeeded;
