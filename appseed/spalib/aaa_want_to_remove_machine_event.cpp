...
{
   FILE * f = fopen(dir::element("machine\\event\\machine_event.bin").c_str(), "rb");
   if(f == NULL)
   {
...
{
   if(!dir::mk(dir::element("machine\\event\\").c_str()))
      return false;
   FILE * f = fopen(dir::element("machine\\event\\machine_event.bin").c_str(), "wb+");
   if(f != NULL)
   {
      fseek(f, 0, SEEK_SET);
      pdata->write(f);
