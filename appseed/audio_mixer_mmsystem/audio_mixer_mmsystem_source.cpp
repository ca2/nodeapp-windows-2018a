...
namespace audio_mixer
{
   ...
   ::multimedia::result source::GetLineControls()
   {
      ...
      if (MMSYSERR_NOERROR != mmrc)
      {
         System.simple_message_box(NULL, MB_OK | MB_ICONEXCLAMATION,
            ...
            m_mixerline.dwLineID, get_device()->m_hMixer, mmrc);
      }
      else
      {
         //        char       ach[128];
         ...
         ::audio_mixer::control *     lpMixerControl;
   ...
   /*::multimedia::result source::GetControl(uint32_t dwControlType, uint32_t dwControlFlags,  ::audio_mixer::control ** ppControl)
   {
   ...
   *ppControl = controla(i);
   ...
   ::multimedia::result source::mixerGetLineInfo(uint32_t dwSource, uint32_t dwDestination, uint32_t fdwInfo)
   {
...
#ifdef WINDOWS
      ::multimedia::result mmrc = ::mixerGetLineInfo(
         (HMIXEROBJ)get_device()->m_hMixer,
         &m_mixerline,
         fdwInfo);
      if (MMSYSERR_NOERROR != mmrc)
      {
         System.simple_message_box(NULL, MB_OK | MB_ICONEXCLAMATION,
            ...
            dwSource, get_device()->m_hMixer, mmrc);
      }
      return mmrc;
#else
        return MMSYSERR_ERROR;
   ...
   ::multimedia::result source::mixerGetLineInfo(uint32_t dwSource, ::audio_mixer::destination * pdestination)
   {
   #ifdef WINDOWS
      ::multimedia::result mmrc = mixerGetLineInfo(dwSource, pdestination->m_mixerline.dwDestination, MIXER_GETLINEINFOF_SOURCE);
