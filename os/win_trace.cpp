...
struct CLASS_DECL_CORE __MAP_MESSAGE
{
   UINT    nMsg;
   const char *  lpszMsg;
};
...
void __trace_message(const char * lpszPrefix, signal_details * pobj)
{
   ...
   {
#ifdef _WIN64
      //TRACE(::core::trace::category_WinMsg, 4, "%s: oswindow=%p, msg = %s (%p, %p)\n",
        // lpszPrefix, pbase->m_oswindow, lpszMsgName,
         //pbase->m_wparam, pbase->m_lparam);
#else
    //  ::OutputDebugString(::core::trace::category_WinMsg, 4, "%s: oswindow=0x%08X, msg = %s (0x%08X, 0x%08X)\n",
//         lpszPrefix, pbase->m_oswindow, lpszMsgName,
  //       pbase->m_wparam, pbase->m_lparam);
#endif
   }
   ...
   {
#ifdef _WIN64
//      ::OutputDebugString(::core::trace::category_WinMsg, 4, "%s: oswindow=%p, msg = 0x%04X (%p, %p)\n",
  //       lpszPrefix, pbase->m_oswindow, lpszMsgName,
    //     pbase->m_wparam, pbase->m_lparam);
#else
//      ::OutputDebugString(::core::trace::category_WinMsg, 4, "%s: oswindow=0x%08X, msg = 0x%04X (0x%08X, 0x%08X)\n",
  //       lpszPrefix, pbase->m_oswindow, lpszMsgName,
    //     pbase->m_wparam, pbase->m_lparam);
#endif
   }
...
void __trace_message(const char * lpszPrefix, LPMSG lpmsg)
{
   ...
   {
#ifdef WIN64
//      TRACE(::core::trace::category_WinMsg, 4, "%s: oswindow=%p, msg = %hs (%p, %p)\n",
  //       lpszPrefix, pMsg->oswindow, lpszMsgName,
    //     pMsg->wParam, pMsg->lParam);
#else
//      ::OutputDebugString(::core::trace::category_WinMsg, 4, "%s: oswindow=0x%08X, msg = %hs (0x%08X, 0x%08X)\n",
