...
namespace win
{
   ...
   stdio_file::stdio_file(sp(::ca2::application) papp) :
      ca2(papp),
      ::win::file(papp)
   {
   ...
   bool stdio_file::open(const char * lpszFileName, UINT nOpenFlags)
   {
      ...
      m_pStream = NULL;
      if (!::win::file::open(lpszFileName, (nOpenFlags & ~::ca2::file::type_text)))
         return FALSE;
   ...
   {
   clearerr(m_pStream);
   vfxThrowFileException(get_app(), ::ca2::file_exception::generic, _doserrno, m_strFileName);
   }
   return lpszResult;
   }*/
