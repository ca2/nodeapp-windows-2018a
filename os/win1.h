...
namespace win
{
   ...
   CLASS_DECL_win void        TimeToFileTime(sp(base_application) papp, const ::datetime::time& time, LPFILETIME pFileTime);
...
CLASS_DECL_CORE int32_t c_cdecl __new_handler(size_t nSize);
...
CLASS_DECL_win LRESULT CALLBACK __window_procedure(oswindow oswindow, UINT nMsg, WPARAM wParam, LPARAM lParam);
...
CLASS_DECL_win void __term_thread(sp(base_application) papp, HINSTANCE hInstTerm = NULL);
...
//CLASS_DECL_win bool ::core::WinInit(__in HINSTANCE hInstance, __in HINSTANCE hPrevInstance,
